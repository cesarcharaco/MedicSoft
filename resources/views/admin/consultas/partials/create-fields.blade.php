...
<div class="form-group{{ $errors->has('id_tipoconsulta') ? ' has-error' : '' }}">
	...
	<option value="">NINGUNA</option>
		@foreach($tipoconsultas as $tipo)
			...
			<option value="{{$tipo->especialidades->tipoconsultas[0]->consultasmontos[0]->id_tipoconsulta}}"> {{$tipo->especialidades->tipoconsultas[0]->consulta}} - Especialidad: {{$tipo->especialidades->especialidad}}</option>
			@endif
		@endforeach
	</select>
</div>
